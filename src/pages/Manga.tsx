    ...
    <>
      ...
      <div className={styles.section}>
        <div className={styles.section__left}>
          <img
            className={styles.section__left__cover}
            src={manga.wrap_path}
            alt="manga-cover"
          />
          <div className={styles.section__left__actions}>
            ...
            <div className={styles.section__left__actions__buttons}>
              {manga.isFavorite ? (
                <LikeFilled onClick={onClickFavorite} />
              ) : (
                <Like onClick={onClickFavorite} />
              )}
              {manga.inCart > 0 ? (
                <>
                  <PlusFilled onClick={onClickPlus} />
                  <MinusFilled onClick={onClickMinus} />
                  <b className={styles.section__left__actions__count}>
                    {manga.inCart}
                  </b>
                </>
              ) : (
        ...
        <div className={styles.section__right}>
          ...
          <p>
            ...
            {manga.genre.map((item, index) => {
              return (
                <span
                  className={styles.section__right__genre}
                  key={`genre${item}`}
                >
                  {item}
            ...
            {similarManga.length > 0
              ? similarManga.map((item, index) => (
                  <Link to={`/manga/${item.id}`} key={`similar${index}`}>
                    <span className={styles.section__right__similar}>
                      {item.title}
                    </span>
